/************************************************************************ 
File name: zuc.c Version:
V1.1 Date: Oct 28,2016
Description: This code provide the implement of ZUC algorithm,which consist of three parts:key stream generation,confidentiality algorithm and integrity algorithm.
Function List:
...
15.ZUC_Integrity            // the ZUC-based integrity 
...
/************************************************************
Function:     GetWord
Description:  get a 32bit word ki from bit strings k[i],k[i+1]...,
              namely ki=k[i]||k[i+1]||…||k[i+31]
Calls:
Called By:    ZUC_Integrity
Input:        k[]:
              i: the position i
Output:
Return:       ki=k[i]||k[i+1]||…||k[i+31]Others:
************************************************************/
...
Input:       LFSR_S:current state of LFSROutput:
Return:      Null
BR_X[]:      achieve X0,X1,X2,X3
...
void ZUC_Init(unsigned char k[], unsigned char iv[], unsigned int LFSR_S[], unsigned int BR_X[], unsigned int F_R[])
{
    ...
    while (count) //32
    {
        ...
        // printf("W:0X%x \n,", W);
        // printf("FR:%x  %x\n,", F_R[0], F_R[1]);
        LFSRWithInitMode(LFSR_S, W >> 1); //利用W >> 1 更新LFSR_S
        printf("%d轮之后LFSR: ", 32 - count);
        for (int i = 0; i < 16; i++)
        {
            printf("%x ", LFSR_S[i]);
        }
...
void ZUC_Confidentiality(unsigned char CK[], unsigned int COUNT, unsigned char BEARER, unsigned char DIRECTION, unsigned int IBS[], int LENGTH, unsigned int OBS[])
{
    ...
    int L, i, t;
    unsigned char iv[16]; //generate vector iv1,iv2,...iv15
    ...
    L = (LENGTH + 31) / 32; //7
    unsigned int *k = (unsigned int *)malloc(sizeof(unsigned int) * L);
...
int ZUC_SelfCheck()
{
    ...
    //     0x137636bd, 0x1d7fa9e0, 0x9efd2007, 0x505906a5, 0xac45dfde, 0xed7757bb, 0xee745749, 0xc2963335, 0x0b ee0ea6,
    ...
    unsigned int message[19] =
        {
            ...
            0x06ca0ea0, 0x7e941b7b, 0xe91348f9, 0xfcb170e2, 0x217fecd9, 0x7f9f68ad, 0xb16e5d7d, 0x21e569d2, 0x80 ed775c,
    ...
    * /
    ...
    // if (memcmp(Keystream, Std_Keystream, KeystreamLen * 4)) // 8 改为 4
    //     return 1;
